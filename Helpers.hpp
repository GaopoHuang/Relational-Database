...
namespace ECE141 {
  ...
  static std::map<std::string,  Keywords> gDictionary = {
    ...
    std::make_pair("indexes",   ECE141::Keywords::indices_kw),
    std::make_pair("indices",   ECE141::Keywords::indices_kw),
  ...
  public:
    ...
    static bool isLogicalOperator(Keywords aKeyword) {
      static Keywords theTypes[]={Keywords::and_kw, Keywords::or_kw, Keywords::not_kw};
      for(auto k : theTypes) {
        if(aKeyword==k) return true;
      }
      return false;
    ...
    static bool typeMatches(DataTypes aType, Value & aValue) {
      size_t theValueIndex = aValue.index();
      switch (aType) {
        case DataTypes::int_type:
          return 1==theValueIndex;
        case DataTypes::float_type:
          return 1==theValueIndex || 2==theValueIndex;
        case DataTypes::varchar_type:
          return 3==theValueIndex;
